...
const ProfileDropdown = ({ isAdmin,profile }) => {
        ...
        <div className="bg-slate-900 w-full h-full rounded-full flex items-center justify-center">
          {
            profile?
            <>
            <img src={profile} className='rounded-full' alt="profile-pic" />
            </>
            :
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {
              username&&<p className="text-sm font-bold text-white">{username ?username: 'Admin User'}</p>
            }
            {
              userMail&&<p className="text-xs font-bold text-slate-400 truncate">{userMail ?userMail :'admin@nexthire.com'}</p>
            }
            ...
            <div className="pt-1 border-t border-slate-800/80">
              ...
              <button
                onClick={handleLogout}
                className="flex items-center gap-3 px-4 py-2 text-sm text-green-400 cursor-pointer hover:text-green-300 hover:bg-red-500/10 w-full text-left transition-colors"
              >
